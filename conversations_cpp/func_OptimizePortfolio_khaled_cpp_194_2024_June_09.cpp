/***
 ...
 khaled_cpp_194_2024_June_09: 476647af-3101-4c52-86ab-5042422d0d51

func_OptimizePortfolio_khaled_cpp_194_2024_June_09

Design a C++ function called OptimizePortfolio that takes an integer num_assets, a vector of doubles const std::vector<double> representing the expected returns of each asset, a vector of doubles const std::vector<double> representing the risks (standard deviations) of each asset, a 2D vector of doubles const std::vector<std::vector<double>> representing the correlation matrix between the assets, and a double risk_tolerance. The function should return a vector of doubles std::vector<double> representing the optimal weights of the assets in the portfolio to maximize the return while minimizing the risk according to the given risk tolerance. Please don't forget to validate the inputs.
...
std::vector<double> OptimizePortfolio(int num_assets, const std::vector<double> &returns,
...
int main() {
    ...
    std::vector<double> result1 =
        OptimizePortfolio(num_assets1, returns1, risks1, correlation_matrix1, risk_tolerance1);
    ...
    std::vector<double> result2 =
        OptimizePortfolio(num_assets2, returns2, risks2, correlation_matrix2, risk_tolerance2);
    ...
    std::vector<double> result3 =
        OptimizePortfolio(num_assets3, returns3, risks3, correlation_matrix3, risk_tolerance3);
    ...
    std::vector<double> result4 =
        OptimizePortfolio(num_assets4, returns4, risks4, correlation_matrix4, risk_tolerance4);
    ...
    std::vector<double> result5 =
        OptimizePortfolio(num_assets5, returns5, risks5, correlation_matrix5, risk_tolerance5);
    ...
    // TEST
    try {
        OptimizePortfolio(-1, {0.1}, {0.1}, {{1.0, 0.5}}, 0.075);
        assert(false);
