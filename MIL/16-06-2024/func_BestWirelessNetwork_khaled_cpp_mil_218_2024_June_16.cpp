/***
 ...
 khaled_cpp_mil_218_2024_June_16: 71c85409-7369-49f0-badb-f02d4482b3b7

 func_BestWirelessNetwork_khaled_cpp_mil_218_2024_June_16
...
Design a C++ function called BestWirelessNetwork that takes an integer num_nodes, a vector of pairs of doubles const
std::vector<std::pair<double, double>> representing the coordinates of the nodes, a vector of doubles const
std::vector<double> representing the transmission power of each node, and a double max_distance. The function should
return a vector of pairs std::vector<std::pair<int, int>> representing the optimal connections between nodes to maximize
the network coverage using Simulated Annealing. Please don't forget to validate the inputs.
...
std::vector<std::pair<int, int>> BestWirelessNetwork(int num_nodes,
                                                         const std::vector<std::pair<double, double>> &coordinates,
                                                         const std::vector<double> &transmission_power,
                                                         double max_distance) {
...
int main() {
    ...
    std::vector<std::pair<int, int>> result1 =
        BestWirelessNetwork(num_nodes1, coordinates1, transmission_power1, max_distance1);
    assert(result1.size() >= 1);
    // TEST_END

    // TEST
    try {
        BestWirelessNetwork(-1, coordinates1, transmission_power1, max_distance1);
        assert(false);
    ...
    // TEST
    try {
        BestWirelessNetwork(num_nodes1, {{0.0, 0.0}}, transmission_power1, max_distance1);
        assert(false);
    } catch (const std::invalid_argument &e) {
        assert(true);
    }
    ...
    // TEST
    try {
        BestWirelessNetwork(num_nodes1, coordinates1, {1.0}, max_distance1);
        assert(false);
    } catch (const std::invalid_argument &e) {
        assert(true);
    }
