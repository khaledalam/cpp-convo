/***
 ...
 khaled_cpp_mil_225_2024_June_16: 38f7e266-4aee-4092-9af7-dc88ea4d4f7a

 func_ImproveDroneDeliveryPath_khaled_cpp_mil_225_2024_June_16
...
Create me a C++ function called ImproveDroneDeliveryPath that takes an integer num_locations, a vector of tuples const
...
double ImproveDroneDeliveryPath(int num_locations, const std::vector<std::tuple<int, int, double>> &paths,
    ...
    for (const std::tuple<int, int, double> &path : paths) {
        int from, to;
        double distance;
    ...
    while (!pqq.empty()) {
        ...
        for (const Path &neighbor : graph[current_location]) {
...
int main() {
    ...
    int end_location1 = 4;
    double result1 = ImproveDroneDeliveryPath(num_locations1, paths1, start_location1, end_location1);
    ...
    int end_location2 = 5;
    double result2 = ImproveDroneDeliveryPath(num_locations2, paths2, start_location2, end_location2);
    ...
    int end_location3 = 3;
    double result3 = ImproveDroneDeliveryPath(num_locations3, paths3, start_location3, end_location3);
    ...
    int end_location4 = 4;
    double result4 = ImproveDroneDeliveryPath(num_locations4, paths4, start_location4, end_location4);
    ...
    int end_location5 = 2;
    double result5 = ImproveDroneDeliveryPath(num_locations5, paths5, start_location5, end_location5);
    assert(std::abs(result5 - 5.0) < 1e-9);
    // TEST_END

    // TEST
    try {
        ImproveDroneDeliveryPath(-1, {{0, 1, 2.0}, {1, 2, 3.0}, {0, 2, 5.0}}, 0, 2);
        assert(false);
    ...
    // TEST
    try {
        ImproveDroneDeliveryPath(4, {}, 0, 2);
        assert(false);
    } catch (const std::invalid_argument &e) {
        assert(true);
    }
    ...
    // TEST
    try {
        ImproveDroneDeliveryPath(4, {{0, 1, 2.0}, {1, 2, 3.0}, {0, 2, 5.0}}, -3, 2);
        assert(false);
    } catch (const std::invalid_argument &e) {
        assert(true);
    }
