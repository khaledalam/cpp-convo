/***
 ...
 khaled_cpp_mil_224_2024_June_16: c66b8ae2-30b7-4970-99a9-f7c088815b98
 
 func_ImproveFlightSchedules_khaled_cpp_mil_224_2024_June_16
...
Design me a C++ function called ImproveFlightSchedules that takes an integer num_flights, a vector of pairs of doubles const std::vector<std::pair<double, double>> representing the start and end times of each flight, and a double min_time_gap. The function should return a vector of integers std::vector<int> representing the optimal sequence of flights to maximize the total number of non-overlapping flights using Tabu Search.
...
std::vector<int> ImproveFlightSchedules(int num_flights, const std::vector<std::pair<double, double>> &flight_times,
                                         double min_time_gap) {
...
int main() {
    ...
    std::vector<int> result1 = ImproveFlightSchedules(num_flights1, flight_times1, min_time_gap1);
    assert(result1.size() == 3);
    // TEST_END

    // TEST
    try {
        ImproveFlightSchedules(-1, flight_times1, min_time_gap1);
        assert(false);
    ...
    // TEST
    try {
        ImproveFlightSchedules(num_flights1, {{0.0, 1.0}}, min_time_gap1);
        assert(false);
    } catch (const std::invalid_argument &e) {
        assert(true);
    }
